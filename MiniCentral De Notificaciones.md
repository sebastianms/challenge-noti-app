...
**13. `AbstractNotification.title/body` eran dead code real en el pipeline (feature 008).** Al diseñar el editor de templates asumimos que `title(context)` y `body(context)` eran invocados en algún punto del despacho. Al leer el código real descubrimos que `EventBuilder` serializa el `context` hash directamente como `payload` en `notification_events`, y `SendgridAdapter` lee `event_payload["subject"]` y `event_payload["body"]` — sin pasar jamás por los métodos de `AbstractNotification`. Los métodos existían como interfaz documentada (con `raise NotImplementedError`) pero no eran llamados por ningún componente del pipeline. Fix: en lugar de parchear el canal, interceptar en `AbstractNotification.send` antes de llamar a `EventBuilder`, enriqueciendo el contexto con `subject:` y `body:` resueltos (override de DB o método Ruby). Esto convirtió los métodos de dead code a punto de extensión real y dejó `SendgridAdapter` sin cambios. Lección: antes de diseñar una capa de override, leer el flujo completo desde la entrada hasta la salida — la interfaz documentada no siempre refleja el camino de ejecución real.
